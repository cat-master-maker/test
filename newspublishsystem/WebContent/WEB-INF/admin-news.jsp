	    ...
	    <div style="width:100%; margin-top:80px;">
    	    <c:forEach items="${news.list}" var="discuss" varStatus="status">
	    	    	<div style="padding-top:20px; padding-bottom:10px;">
		    	    	...
		    	    	<div style="width:100%;">${discuss.content}<span style="color:red" onclick="del(${discuss.id},${news.id })">删除</span></div>
		    	    	...
		    	    	<c:forEach items="${discuss.list}" var="discusssub" varStatus="statussub">
			    	    	<div style="margin-left:50px; margin-top:30px; border-left:5px solid #999; padding-left:10px;">
		    	    			...
		    	    			<div style="width:100%;">${discusssub.content} <span  onclick="del(${discuss.id},${news.id })">删除</span></div>
...
<script>
	...
	function discuss(type, newsId, id){
		...
		$.ajax({
			...
			success	:function(data){
				location.href = "coreServlet?bizCode=4type=del&id="+ newsId;
			}
		});
	}
	...
	function del(id,newsid){
		layer.confirm('确定删除？', {icon:3, title:'提示'}, function(index){
			location.href = "coreServlet?bizCode=8&id="+ id +"&newsid="+newsid;
			layer.close(index);
		});
	}
</script> 
