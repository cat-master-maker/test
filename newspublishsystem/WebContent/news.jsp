...
<body style="padding-bottom:50px;">
	...
	<div class="discuss" style="width:762px; margin:0 auto; margin-top:140px;">
		<p style="color:#666; font-size:14px;"><i class="layui-icon" style="font-size: 15px; color: #20B2AA;">&#xe642;</i>${user.status }留言</p>
	    ...
	    <button style="float:right;" id="dis-btn" class="layui-btn layui-btn-primary" onclick="discuss(0,${news.id},${news.id},${user.status})">提交</button>
    	    ...
    	    <c:forEach items="${news.list}" var="discuss" varStatus="status">
		    	...
		    	<div id="replydiv${discuss.id}" hidden>
		    		...
		    		<div style="width:100%; height:50px;">
		    			<button style="float:right; margin-top:15px;" id="dis-btn" class="layui-btn layui-btn-primary" onclick="discuss(1,${news.id},${discuss.id},${user.status})">提交</button>
		    		</div>
		    </div>
    	    </c:forEach>
...
<script>
	...
	function pageTurn(id){
		location.href = "coreServlet?bizCode=4&type=comm&id="+ id;
	}
	
	//评论
	function discuss(type, newsId, id,status){//加一个参数 传递user的状态值    action中可写若username有值，且status不为2（拉黑，类中status赋值为1，onclick判断status，null或者2不能发表评论）
		...
		if(status == 1){
			...
			$.ajax({
				...
				success	:function(data){
					location.href = "coreServlet?type=comm&bizCode=4&id="+ newsId;
				}
			});
